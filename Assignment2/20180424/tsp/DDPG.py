...
class ReplayMemory :
    ...
    def sample(self, batch_size) :
        ...
        state, action_1, action_2, reward, next_state, done = zip(*transitions)

        state = torch.cat(state)
        action_1 = torch.tensor(action_1, dtype=torch.float32).reshape(-1, 1)
        action_2 = torch.tensor(action_2, dtype=torch.float32).reshape(-1, 1)
...
class Gaussian_Noise :
    ...
    def get_action(self, action) :
        ...
        self.noise = np.random.normal(0, 5, 1) * self.epsilon
        
        self.epsilon -= self.epsilon_decay
        if self.epsilon < self.epsilon_min:
            self.epsilon = self.epsilon_min

        action_before_norm = np.clip(action + self.noise, -0.1, 0.1)

        return action_before_norm
...
class DDPGAgent(nn.Module) :
    ...
    def get_action(self, state, t=0) :
        ...
        actions_before_norm = self.actor(state).detach().numpy()
        #action_after_norm_1 = self.gaussian_noise.get_action(actions_before_norm[0][0])
        #action_after_norm_2 = self.gaussian_noise.get_action(actions_before_norm[0][1])
        
        action_after_norm_1 = (actions_before_norm[0][0] + 1)/2 * (self.action_max - self.action_min) + self.action_min
        action_after_norm_2 = (actions_before_norm[0][1] + 1)/2 * (self.action_max - self.action_min) + self.action_min

        return int(action_after_norm_1), int(action_after_norm_2)
